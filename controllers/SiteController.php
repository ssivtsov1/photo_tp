...
//        Создаем вид на сервере PostGres, откуда берем список подстанций
  ...
  ORDER BY eq.name_eqp";
    ...
    public function actionDetail_photo($id)
    {
    ...
    // Редактирование описания фото
    public function actionEditimg($img,$descr,$date,$nazv_tp,$id)
    {
        $model = new cphoto();
        if ($model->load(Yii::$app->request->post())) {
    ...
    // Удаление фото
    public function actionDel_img($id)
    {
        $data = edit_photo::find()->where(['id'=>$id])->one();
...
//            'date'=> $model->date,'priz_description'=>$model->priz_description]);
    ...
    // Просмотр фото
    public function actionViewphoto($id_res,$id_tp,$descr,$date,$priz_description)
    {
...
//
        ...
        if(empty($data[0]->id))
        {
            ...
            return $this->render('viewphoto', [
                ...
                'date'=> $date,'priz_description'=>$priz_description
            ]);
...
//            $tp = spr_tp::findBySql($sql)->all();
    ...
    public function actionGet_tp($name)
    {
        ...
        if (Yii::$app->request->isAjax) {
            ...
            $id = $tp[0]->id;
            $geo = geo_tp::find()->select(['lat','lon'])->where('id_tp=:id',[':id' => $id])->all();
            if(isset($geo[0]->lat))
                $lat = $geo[0]->lat;
            else
                $lat = '';
...
//                else
                    ...
                    group by id_tp  
                    union Select '-99999    Всі підстанції    (".$count.")'". " as nazv order by substr(nazv,7) ";
